...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event //点击开始
{
    if(imageViewArr.count > 0)
    {
        ...
        if(index < 0)
            index = 0;
        if(index > (((int)imageViewArr.count) - 1))
        {
            index = (((int)imageViewArr.count) - 1);
        }
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _moveView = [[UIView alloc]initWithFrame:CGRectMake(30.0, 20.0, (SCREEN_WIDTH-60.0), (500-40.0))];
        _scrollView = [[SlideScrollView alloc]initWithFrame:CGRectMake(30.0, 20.0, (SCREEN_WIDTH-60.0), (500-40.0))];
...
{
    [self setFrame:frame];
    [_moveView setFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
    [_scrollView setFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
}

-(void)ZMarginValue:(float)zMarginValue XMarginValue:(float)xMarginValue AngleValue:(float)angleValue Alpha:(float)alphaValue
{
...
- (void)loadScrollView //加载滚动视图数据
{
    ...
    for(int i=0; i<_imageArray.count; i++)
    {
        ...
        [imageView setLayerCornerRadius:7.0];
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView //滚动时处理
{
    ...
    for(int i=0; i<_imageViewArray.count; i++)
    {
        ...
        [moveImageView setLayerCornerRadius:7.0];
    }
